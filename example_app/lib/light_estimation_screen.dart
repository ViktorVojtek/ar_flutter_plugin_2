...
/// Light Estimation Example Screen
/// Demonstrates real-time lighting condition monitoring in AR
class LightEstimationScreen extends StatefulWidget {
  const LightEstimationScreen({Key? key}) : super(key: key);

  @override
  State<LightEstimationScreen> createState() => _LightEstimationScreenState();
}

class _LightEstimationScreenState extends State<LightEstimationScreen> {
  ...
  void _onARViewCreated(
    ...
    // IMPORTANT: Wait a moment for AR session to initialize before starting monitoring
    // This ensures arFrame is available for light estimation
  ...
  void _onLightingChanged(Map<String, dynamic> lightData) {
    print('🎨 Widget: _onLightingChanged called with data: $lightData');
    print('🎨 Widget: mounted = $mounted');
    
    if (!mounted) {
      print('🎨 Widget: Skipping - not mounted');
      return;
    }

    print('🎨 Widget: Calling setState...');
    setState(() {
      ...
      print('🎨 Widget: setState - intensity: $_lightIntensity, low: $_isLowLight, veryLow: $_isVeryLowLight');
      ...
      print('🎨 Widget: setState - status: $_lightingStatus');
    });
    print('🎨 Widget: setState completed');
  ...
  Future<void> _startMonitoring() async {
    setState(() {
      _lightingStatus = 'Starting monitoring...';
    ...
    setState(() {
      _isMonitoring = true;
      _lightingStatus = 'Monitoring... waiting for data';
      _statusColor = Colors.blue.shade700;
    });
    
    debugPrint('💡 Light monitoring started - callbacks should arrive every 1 second');
  }
